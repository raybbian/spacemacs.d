...
before packages are loaded."
  ;; `SPC m ==' still formats on demand; only the on-save path moved to LSP.
  (setq clang-format-style "file"
        clang-format-fallback-style "LLVM")

  ;; cc-mode applies a style during mode init, which clobbers a bare
  ;; `c-basic-offset' set from a mode hook. Registering a style and selecting it
  ;; via `c-default-style' is the supported way to change electric indentation.
  ;; Mirrors the .clang-format in use: LLVM base, IndentWidth 4, no namespace
  ;; indent, access labels outdented one step.
  (c-add-style "clang-format-llvm"
               '("stroustrup"
                 (c-basic-offset . 4)
  ...
  (custom-set-variables
   ...
   '(package-selected-packages
     '(ace-link aggressive-indent all-the-icons auto-compile auto-highlight-symbol
                ...
                indent-guide info+ inspector link-hint lorem-ipsum lsp-docker
                lsp-mode lsp-origami lsp-treemacs lsp-ui macrostep markdown-mode
